...
static bark_sequence bark_tokenize_input(
                      const struct gpt_model & model,
                     const struct bark_vocab & vocab,
...
static bool bark_eval_text_encoder_internal(
    ...
    struct ggml_cgraph * gf = bark_build_gpt_graph(model, allocr, input, n_past, merge_ctx);
...
int main(void) {
    ...
    const std::string model_path = "../ggml_weights/ggml_weights_text.bin";
    const std::string vocab_path = "../ggml_weights/ggml_vocab.bin";
    ...
    // load the model and vocab
    {
        ...
        if (!gpt_load_model_weights(model_path, model)) {
        ...
        if (!bark_vocab_load(vocab_path, vocab, expected_size)) {
    ...
    // allocate the compute buffer
    {
        ...
        struct ggml_cgraph * gf = bark_build_gpt_graph(
            model, allocr, std::vector<bark_token>(n_tokens, 0), n_past, true);
    ...
    bark_sequence tokens = bark_tokenize_input(model, vocab, prompt);
    ...
    for (int i = 0; i < N_STEPS_TEXT_ENCODER; i++) {
        {
            ...
            if (!bark_eval_text_encoder_internal(model, allocr, input, logits, n_past, true, n_threads)) {
        ...
        bark_token next = gpt_multinomial_sample(logits, rng, 0.6 /* temp */, &eos_p);

        if (next == SEMANTIC_VOCAB_SIZE || eos_p >= 0.2 /* min_eos_p */) {
            break;
        }
    ...
    // report timing
    {
        ...
        printf("%s:  predict time = %8.2f ms / %.2f ms per token\n", __func__, t_predict_us/1000.0f, t_predict_us/1000.0f/n_sample);
