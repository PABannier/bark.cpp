...
inline static void ggml_vec_sig_f32  (const int n, float * y, const float * x) { for (int i = 0; i < n; ++i) y[i] = 1.f / (1.f + expf(-x[i])); }
...
struct ggml_tensor * ggml_l2_norm_impl(
        struct ggml_context * ctx,
        struct ggml_tensor  * a,
        bool inplace) {
    bool is_node = false;
...
struct ggml_tensor * ggml_l2_norm(
        struct ggml_context * ctx,
        struct ggml_tensor  * a) {
    return ggml_l2_norm_impl(ctx, a, false);
}

struct ggml_tensor * ggml_l2_norm_inplace(
        struct ggml_context * ctx,
        struct ggml_tensor  * a) {
    return ggml_l2_norm_impl(ctx, a, true);
}
...
struct ggml_tensor * ggml_conv_1d(
    ...
    const int64_t lout = floorf(((b->ne[0] - (a->ne[0] - 1) - 1) / stride) + 1);
    const int64_t ne[4] = { lout, a->ne[2], 1, 1, };
    struct ggml_tensor * result = ggml_new_tensor(ctx, GGML_TYPE_F32, 2, ne);

    struct ggml_tensor * c = ggml_new_tensor_1d(ctx, GGML_TYPE_I32, 1);
    ((int32_t *) c->data)[0] = stride;
...
struct ggml_tensor * ggml_transpose_conv_1d(
        struct ggml_context * ctx,
        struct ggml_tensor  * a,
        struct ggml_tensor  * b,
        int                   stride) {
    ...
    const int64_t lout = (b->ne[0] - 1) * stride + a->ne[0];
    const int64_t ne[4] = { lout, a->ne[1], 1, 1, };
    struct ggml_tensor * result = ggml_new_tensor(ctx, GGML_TYPE_F32, 2, ne);
    struct ggml_tensor * c = ggml_new_tensor_1d(ctx, GGML_TYPE_I32, 1);

    ((int32_t *) c->data)[0] = stride;

    result->op     = GGML_OP_TRANSPOSE_CONV_1D;
    result->grad   = is_node ? ggml_dup_tensor(ctx, result) : NULL;
    result->src0   = a;
    result->src1   = b;
    result->opt[0] = c;
...
struct ggml_tensor * ggml_pad_1d_constant(
        struct ggml_context * ctx,
        struct ggml_tensor  * a,
        int                   padding[2],
        float                 v) {
    ...
    struct ggml_tensor * b = ggml_new_tensor_1d(ctx, GGML_TYPE_I32, 3);
    ((int32_t *) b->data)[0] = padding[0];
    ((int32_t *) b->data)[1] = padding[1];
    ((int32_t *) b->data)[2] = v;
    ggml_set_name(b, "pad_left, pad_right, v");

    const int64_t ne[4] = { padding[0]+padding[1]+a->ne[0], a->ne[1], 1, 1, };
    struct ggml_tensor * result = ggml_new_tensor(ctx, GGML_TYPE_F32, 2, ne);

    result->op   = GGML_OP_PAD_1D_CONST;
    result->grad = is_node ? ggml_dup_tensor(ctx, result) : NULL;
    result->src0 = a;
    result->src1 = b;
...
// ggml_pad_1d_reflective

struct ggml_tensor * ggml_pad_1d_reflective(
        struct ggml_context * ctx,
        struct ggml_tensor  * a,
        int                   padding[2]) {
    bool is_node = false;
    ...
    ((int32_t *) b->data)[0] = padding[0];
    ((int32_t *) b->data)[1] = padding[1];
    ggml_set_name(b, "pad_left, pad_right");

    const int64_t ne[4] = { padding[0]+padding[1]+a->ne[0], a->ne[1], 1, 1, };
    struct ggml_tensor * result = ggml_new_tensor(ctx, GGML_TYPE_F32, 2, ne);

    result->op   = GGML_OP_PAD_1D_REFLEC;
    result->grad = is_node ? ggml_dup_tensor(ctx, result) : NULL;
    result->src0 = a;
    result->src1 = b;
...
static void ggml_compute_forward_argmax_f32(
    ...
    for (int64_t i1 = 0; i1 < ne01; i1++) {
        ...
        int tmp = 0;
        ggml_vec_argmax_f32(ne00, &tmp, src);
        dst_[0] = tmp;
    }
}
...
static void ggml_compute_forward_set(
    ...
    switch (src0->type) {
        ...
        case GGML_TYPE_I32:
            {
                ggml_compute_forward_set_i32(params, src0, src1, opt0, dst);
            } break;
        case GGML_TYPE_F16:
        case GGML_TYPE_Q4_0:
        case GGML_TYPE_Q4_1:
...
static void ggml_compute_forward_conv_1d(
    ...
    switch (src0->type) {
        ...
        case GGML_TYPE_F32:
            {
                ggml_compute_forward_conv_1d_f32(params, src0, src1, opt0, dst);
            } break;
        default:
            {
                GGML_ASSERT(false);
...
// ggml_compute_forward_transpose_conv_1d

static void ggml_compute_forward_transpose_conv_1d_f32(
        const struct ggml_compute_params * params,
        const struct ggml_tensor * src0,
        const struct ggml_tensor * src1,
        const struct ggml_tensor * opt0,
...
static void ggml_compute_forward_transpose_conv_1d(
        const struct ggml_compute_params * params,
        const struct ggml_tensor * src0,
        const struct ggml_tensor * src1,
        const struct ggml_tensor * opt0,
    ...
    switch (src0->type) {
        case GGML_TYPE_F32:
            {
                ggml_compute_forward_transpose_conv_1d_f32(params, src0, src1, opt0, dst);
            } break;
        default:
            {
                GGML_ASSERT(false);
...
// ggml_compute_forward_pad_1d_const

static void ggml_compute_forward_pad_1d_const_f32(
        const struct ggml_compute_params * params,
        const struct ggml_tensor * src0,
        const struct ggml_tensor * src1,
        struct ggml_tensor * dst) {
    ...
    const int  pl = ((int32_t *) src1->data)[0];
    const int  pr = ((int32_t *) src1->data)[1];
    const float v = ((int32_t *) src1->data)[2];
    ...
    for (int i1 = 0; i1 < ne1; i1++) {
        ggml_vec_set_f32(pl, (float *) ((char *) dst->data + i1*nb1), v);

        ggml_vec_cpy_f32(ne00,
                (float *) ((char *)  dst->data + i1*nb1 + pl*nb0),
                (float *) ((char *) src0->data + i1*nb01));

        ggml_vec_set_f32(pr, (float *) ((char *) dst->data + i1*nb1 + (pl+ne0)*nb0), v);
    }
}

static void ggml_compute_forward_pad_1d_const(
        const struct ggml_compute_params * params,
        const struct ggml_tensor * src0,
        const struct ggml_tensor * src1,
        struct ggml_tensor * dst) {
    switch (src0->type) {
        case GGML_TYPE_F32:
            {
                ggml_compute_forward_pad_1d_const_f32(params, src0, src1, dst);
            } break;
        default:
            {
                GGML_ASSERT(false);
...
// ggml_compute_forward_pad_1d_reflec

static void ggml_compute_forward_pad_1d_reflec_f32(
        const struct ggml_compute_params * params,
        const struct ggml_tensor * src0,
        const struct ggml_tensor * src1,
        struct ggml_tensor * dst) {
    ...
    for (int i1 = 0; i1 < ne1; i1++) {
        float * pl_ptr = (float *) ((char *) dst->data + i1*nb1 +       pl*nb0);
        float * pr_ptr = (float *) ((char *) dst->data + i1*nb1 + (ne0-pr-1)*nb0);

        ggml_vec_cpy_f32(ne00, pl_ptr, (float *) ((char *) src0->data + i1*nb01));

        for (int i0 = 1; i0 <= pl; i0++) {
            pl_ptr[-i0] = pl_ptr[i0];
        }

        for (int i0 = 1; i0 <= pr; i0++) {
            pr_ptr[i0] = pr_ptr[-i0];
        }
    }
...
static void ggml_compute_forward_pad_1d_reflec(
        const struct ggml_compute_params * params,
        const struct ggml_tensor * src0,
        const struct ggml_tensor * src1,
        struct ggml_tensor * dst) {
    switch (src0->type) {
        case GGML_TYPE_F32:
            {
                ggml_compute_forward_pad_1d_reflec_f32(params, src0, src1, dst);
            } break;
        default:
            {
                GGML_ASSERT(false);
...
static void ggml_compute_forward(struct ggml_compute_params * params, struct ggml_tensor * tensor) {
    ...
    switch (tensor->op) {
        ...
        case GGML_OP_TRANSPOSE_CONV_1D:
            {
                ggml_compute_forward_transpose_conv_1d(params, tensor->src0, tensor->src1, tensor->opt[0], tensor);
            } break;
        case GGML_OP_PAD_1D_CONST:
            {
                ggml_compute_forward_pad_1d_const(params, tensor->src0, tensor->src1, tensor);
            } break;
        case GGML_OP_PAD_1D_REFLEC:
            {
                ggml_compute_forward_pad_1d_reflec(params, tensor->src0, tensor->src1, tensor);
            } break;
