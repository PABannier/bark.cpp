...
inline static void ggml_vec_sigm_f32 (const int n, float * y, const float * x) { for (int i = 0; i < n; ++i) y[i] = 1.f / (1.f + expf(-x[i]));  }
...
GGML_API struct ggml_tensor * ggml_conv_1d(
    ...
    const int64_t ne[4] = {
        ggml_calc_conv_output_size(b->ne[0], a->ne[0], s0, 0 /*p0*/, 1 /*d0*/),
        a->ne[2], 1, 1,
    };
...
static void ggml_compute_forward_conv_transpose_1d_f16_f32(
    ...
    int32_t s0 = ((int32_t *)(dst->op_params))[0];
...
static void ggml_compute_forward_unary(
    ...
    switch (op) {
        ...
        case GGML_UNARY_OP_SIGMOID:
            {
                ggml_compute_forward_sigmoid(params, src0, dst);
            } break;
